...
import { Alert, Platform, ActivityIndicator, AppState } from 'react-native';
...
function App() {
  useEffect(() => {
    ...
    const handleLocationBasedTask = async (remoteMessage) => {
      ...
      if (locationPermission === RESULTS.GRANTED) {
        ...
        if (location) {
          ...
          // Log location immediately when notification arrives
          console.log("Immediate location logged:", {
            latitude: location.coords.latitude,
            longitude: location.coords.longitude,
          });
          // Uncomment the following line when ready to send location to API
    ...
    // Set up handlers for receiving push notifications in foreground and background
    messaging().setBackgroundMessageHandler(handleBackgroundMessage);
    const unsubscribe = messaging().onMessage(async (remoteMessage) => {
      console.log('FCM Message Data:', remoteMessage);
      // Log the location to the console as soon as the notification is received
      await handleLocationBasedTask(remoteMessage);
    });
